...
function UI:show(settingName)
	...
	Setting.Task.EnableAutoProcessForkTask=result["UnitTaskRunEnable"]["自动完成支线任务"]
...
function UI:GetSetting(result,id)
	for k,v in pairs(result) do
		...
		if startIndex>0 then
			...
			elseif itemCity=="Value" then
				Setting.Building[settingInfo[1].."Setting"].Value[settingInfo[3]]=tonumber(v)
			else
				local selectValue=v=="0" and true or false
				Setting.Building[settingInfo[1].."Setting"][settingInfo[2]]=selectValue
				sysLog("Setting.Building."..settingInfo[1].."Setting."..settingInfo[2].."="..tostring(selectValue))
			end
		end
	end
end
...
function UI:BuildCityDevelopPriortyOption(p,TabId)
	...
	p:addCheckBoxGroup_single(2.5,1,TabId..".EnableAutoDevelop","0","建设")
	p:addCheckBoxGroup_single(2.5,1,TabId..".EnableAutoProductSupply","","生产补给")
	p:addCheckBoxGroup_single(2.5,1,TabId..".EnableAutoRepair","","修理")
	p:addCheckBoxGroup_single(2.5,1,TabId..".EnableAutoConcilite","","安抚")
	p:newLine()
	p:addCheckBoxGroup_single(9,1,TabId..".SkipWhenHigherPriorityBuilingIsLackOfRescource","0","当优先级更高的建筑缺少资源时暂停其他建筑升级")
	p:newLine()
	p:addCheckBoxGroup_single(3,1,TabId..".EnableAutoImmediateBuilding","","水晶秒建筑")
	p:addLabel(0.6,1.2,"最低",20)
	p:addEdit(2,1,TabId..".Value.MinImmediatePrice","11","number")
	p:addLabel(0.6,1.2,"最高",20)
	p:addEdit(2,1,TabId..".Value.MaxImmediatePrice","20","number")
	p:newLine()
	p:newLine()
	p:addCheckBoxGroup_single(2.5,1,TabId..".EnableCityDevelop","0","城市建设")
	p:newLine()
	nowLineOptNum=0
	...
	p:newLine()
	p:newLine()
	p:addCheckBoxGroup_single(2.5,1,TabId..".EnableFieldDevelop","0","野地建设")
	p:newLine()
	nowLineOptNum=0
...
function UI:BuildGeneralPage(ui)
	...
	p:newLine()
	p:addCheckBoxGroup(8,4,"UnitTaskRunEnable","0@1@2@3@4@5@6@7@8",{"处理活动页","自动结束任务","自动完成主线任务","自动完成支线任务","防重复执行主线","收集野地事件","主动收集野地事件"})
...
function UI:BuildSkillPage(ui)
	...
	p:addComboBox(2,1,"Skill.ReleaseWhenFull","",{"达到上限","30","50","60","90","100","120","140","160","180"})
	local whenFullSkills=Skills
	table.insert(whenFullSkills,"用户队列")
	p:addComboBox(3,1,"Skill.SkillReleaseWhenFull","",Skills)
	p:newLine()
	...
	for i=1,queueNum do
		BuildSkillQueueSelect(i,getNumberConfig("Skill.Queue"..i..".SkillIndex",0))
		p:newLine()
	end
end
